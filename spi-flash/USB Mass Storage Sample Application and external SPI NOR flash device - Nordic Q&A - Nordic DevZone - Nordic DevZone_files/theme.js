(function ($) {
	...
	/*
       Custom inline loader function
       @visible: true if the loader should be displayed in 'this' element.
       @restore: The text to display after it's done loading. If not specified, it will display the previous text.
    */
	...
	$.fn.postable = function(url, dataFunc, successFunc, dataExtension, errorFunc){
		...
		return $(this).on('click', function(){
			...
			try{
				// Get the data to send and add the ticket ID:
				var data = dataFunc ? dataFunc(button) : {};
				if(dataExtension) {
					for(var key in dataExtension) {
						data[key] = dataExtension[key];
					}
				}
			}catch(e){
				// Validation failure
				ev.notifications.show(e.message, { type: 'error' });
				return;
			}
			...
			ev.post({
				...
				success: function(result){
					...
					if(successFunc){
						...
						$.ns.modal && $.ns.modal.close(result);
					}
				},
				error: function(e){
					console.error(e);
					...
					if(e.responseJSON && e.responseJSON.Errors) {
						ev.notifications.show(e.responseJSON.Errors[0], { type: 'error' });
					}
...
$(document).ready(function () {
	...
	$('.burger-menu').on('click', function () {
		if ($(this).find('.content-list:visible').length == 0) {
			$(this).addClass('Open')
		} else {
	...
	if ($(window).width() > 670) {
		tileHeight();
		$(window).on('resize', tileHeight);
	}
});


function tileHeight() {
	var $list = $('.welcome-fudge'),
		$items = $list.find('.tile');
