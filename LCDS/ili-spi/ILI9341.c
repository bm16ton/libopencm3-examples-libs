...
gpio_set_mode(GPIOB, GPIO_MODE_OUTPUT_50_MHZ ,GPIO_CNF_OUTPUT_ALTFN_PUSHPULL, GPIO13 |  GPIO15);  //F103
gpio_set_mode(GPIOB, GPIO_MODE_INPUT ,GPIO_CNF_INPUT_FLOAT, GPIO14);  //F103

//spi initialization;
spi_set_master_mode(SPI2);
//spi_set_baudrate_prescaler(SPI2, SPI_CR1_BR_FPCLK_DIV_64);  // Działa OK
...
/* Setup GPIO pins for AF5 for SPI1 signals. */
gpio_mode_setup(GPIOB, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO13 |  GPIO15);  //L152
gpio_mode_setup(GPIOB, GPIO_MODE_INPUT, GPIO_PUPD_NONE , GPIO14); // L152
gpio_set_af(GPIOB, GPIO_AF5, GPIO13 | GPIO14 | GPIO15); //L152
...
void fillScreen(uint16_t XL, uint16_t XR, uint16_t YU, uint16_t YD, uint16_t color)
{
    ...
    for(i=0; i < XY; i++)
    {
        //SPI.transfer(Hcolor);
        //SPI.transfer(Lcolor);
        spi_xfer(SPI2, Hcolor);
		/* Wait for transfer finished. */
		while (!(SPI_SR(SPI2) & SPI_SR_RXNE));
		spi_xfer(SPI2, Lcolor);
		/* Wait for transfer finished. */
		while (!(SPI_SR(SPI2) & SPI_SR_RXNE));
    }
